...
class DownloadAppsTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        dbKey = "enilepiP"
        useDHIS = True
        cls.pl = Pipeline(dbFileName, dbDirectory, dbKey, useDHIS)
...
class Check_storeResultsDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.xferDB = TransferDB(dbFileName = self.dbFileName,
                                 dbDirectory = self.dbDirectory,
                                 dbKey = self.dbKey,
                                 plRunDate = self.pl.pipelineRunDate)
...
class Check_Pipeline_runOpenVA_InSilicoVA(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        conn.commit()
        conn.close()
        pl = Pipeline(dbFileName, dbDirectory, dbKey, True)
        settings = pl.config()
        settingsPipeline = settings["pipeline"]
        settingsODK = settings["odk"]
        settingsOpenVA = settings["openVA"]
...
class Check_runOpenVA_SmartVA(unittest.TestCase):
    ...
    def test_runOpenVA_SmartVA_completed(self):
        """Check that runOpenVA() creates an R script for SmartVA:"""
        self.assertEqual(self.rOut["returncode"], 0)
...
class Check_Pipeline_cleanPipeline(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        nowDate = datetime.datetime.now()
        pipelineRunDate = nowDate.strftime("%Y-%m-%d_%H:%M:%S")
        pl = Pipeline("copy_Pipeline.db", ".", "enilepiP", True)
        pl.closePipeline()
        cls.pl = pl
