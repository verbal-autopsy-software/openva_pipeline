...
class Pipeline:
    ...
    def config(self):
        ...
        if self.useDHIS:
            ...
            self.dhisOrgUnitCodes = xferDB.query_columns_to_dataframe(conn,
                                                                      'DHIS_ORGUNIT_CODES',
                                                                      ['orgUnit', 'odkCode', 'dhisCode'])
            conn.close()
            return(settings)
    ...
    def runDHIS(self, argsDHIS, argsPipeline, orgUnitCodes):
        ...
        pipelineDHIS = DHIS(argsDHIS, argsPipeline.workingDirectory, orgUnitCodes)
